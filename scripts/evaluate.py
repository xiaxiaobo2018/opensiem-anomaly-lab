...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the AnoMod anomaly baseline.")
    parser.add_argument(
        "--with-explanations",
        action="store_true",
        help="Generate LLM explanations for top anomalous scenarios after evaluation.",
    )
...
def main() -> None:
    ...
    if args.with_explanations:
        ...
        json_path, markdown_path = save_explanation_artifacts(records)
        print(f"\nExplanation JSON: {json_path}")
        print(f"Explanation Markdown: {markdown_path}")
